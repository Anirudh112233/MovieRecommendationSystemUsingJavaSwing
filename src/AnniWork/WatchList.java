...
public class WatchList extends javax.swing.JFrame {
    ...
    private void loadWatchlistFromDatabase() {
        ...
        try (Connection conn = Connect.ConnectToDB()) {
            String query = "SELECT movie_name FROM watchlist WHERE emailid = ?";
    ...
    try (Connection conn = Connect.ConnectToDB()) {
        ...
        String checkQuery = "SELECT COUNT(*) FROM watchlist WHERE emailid = ? AND movie_name = ?";
        ...
        String query = "INSERT INTO watchlist (emailid, movie_name) VALUES (?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, loggedInEmail);
    ...
    private void removeMovieFromWatchlist(String movieName) {
        try (Connection conn = Connect.ConnectToDB()) {
            String query = "DELETE FROM watchlist WHERE emailid = ? AND movie_name = ?";
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                stmt.setString(1, loggedInEmail);
                stmt.setString(2, movieName);
                stmt.executeUpdate();
